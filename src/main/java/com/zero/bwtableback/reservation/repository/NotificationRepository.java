...
public interface NotificationRepository extends JpaRepository<Notification, Long> {
    ...
    // 고객 회원이 받은 알림 조회
    Page<Notification> findByReservation_Member_IdAndStatusOrderBySentTimeDesc(
            Long memberId, NotificationStatus status, Pageable pageable);

    // 가게 주인이 받은 알림 조회
    Page<Notification> findByReservation_Restaurant_Member_IdAndStatusOrderBySentTimeDesc(
            Long memberId, NotificationStatus status, Pageable pageable);
